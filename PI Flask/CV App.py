...
def region_of_interest2(img, verticesL, verticesR, color3=(255,255,255), color1=255):#ROI
    ...
    return ROI_image
def draw_lines(img, lines, color, thickness, h_width, RL): # 선 그리기
    global left, right
    for line in lines:
        for x1,y1,x2,y2 in line:
            if h_width > x1 and RL==True:
                ...
                right=1
                #cv2.line(img, (x1, y1), (x2, y2), color, thickness)
def weighted_img(img, initial_img, a=1, b=1., t=0.): # 두 이미지 operlap 하기
...
class Line_Detection(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            #temp= line_img
            left=right=0
        #frame = weighted_img(temp, frame) # 원본 이미지에 검출된 선 overlap
